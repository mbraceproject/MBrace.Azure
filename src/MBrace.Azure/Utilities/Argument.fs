...
with
    static member ToBase64Pickle (config : Config, isClientInstance : bool) =
        Config.Initialize(populateDirs = true, isClientInstance = isClientInstance)
        let pickle = VagabondRegistry.Instance.Serializer.Pickle(config)
        System.Convert.ToBase64String pickle

    static member OfBase64Pickle (args : string [], isClientInstance : bool) =
        Config.Initialize(populateDirs = true, isClientInstance = isClientInstance)
